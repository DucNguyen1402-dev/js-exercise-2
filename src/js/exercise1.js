...
function resetBenchmarkWarningArea() {
  ...
  DOM.benchmarkWarningArea.classList.remove("bg-rose-500");
  DOM.benchmarkWarningArea.textContent = "";
}

function resetBenchmarkInput() {
  DOM.benchmarkInput.classList.remove("ring-1", "ring-rose-500");
...
function invalidBenchmark() {
  DOM.benchmarkWarningArea.classList.toggle("hidden");
...
function checkAdmission() {
  ...
  resetCheckArea();
  if (finalScore >= benchmarkValue) {
    DOM.checkArea.classList.add("bg-green-500");
    DOM.happyIcon.classList.remove("hidden");
    DOM.checkAreaText.textContent = `Congratulations! You passed. Your score: ${finalScore}`;
  } else {
    DOM.checkArea.classList.add("bg-yellow-500");
    DOM.sadIcon.classList.remove("hidden");
    DOM.checkAreaText.textContent = `Unfortunately, you did not pass. Your score: ${finalScore}`;
  }
}
